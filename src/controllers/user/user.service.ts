...
import { IUser, ILoginCredentials, IUserCreateParams } from '../../interfaces/user'
...
export const userService = {
  loginUser: async (credentials: ILoginCredentials) : Promise<IUser | undefined> => {
    const user = await dbService.queryOne(
      ...
      [credentials.email, credentials.password]) as IUser
    return user
  },
  ...
  createUser: async (params: IUserCreateParams) : Promise<IUser | undefined> => {
    const user = await dbService.queryOne(`
      ...
      [params.name, params.email, params.password, params.privileges]) as IUser
    return user
  }
}
